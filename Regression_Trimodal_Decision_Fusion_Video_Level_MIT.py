# -*- coding: utf-8 -*-
"""Decision_Fusion_video_level_au_kin_audio_with_function_O.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1BK7_M0jmZEKGJ6Ra12yRyt9cwnbZDna3
"""
...
def model_call(file_to_work, chunk_size,l_name, Model_kineme, Model_AU, Model_audio, callback,x,y,z,k1):
  seqLen = chunk_size-1
  ...
  for train_idx, test_idx in rkf.split(file_to_work):
    ...
    for i in range(0,len(test_data)):
        ...
        y1 = np.mean(final_test_pred)   #Voting for classification to change
    ...
    final_train_pred1 = x*train_pred_kineme + y*train_pred_au + z*train_pred_audio
    ...
    y_pred_train = np.around(final_train_pred1,3)
    


    test_mae.append(1-mean_absolute_error(test_labels, y_pred_video)) #mean squarred test error
    # print(test_mae)               
    y1 = np.array(test_labels).reshape(-1,1)
    a = np.corrcoef(y1.T,np.array(y_pred_video).T)
  ...
  path = '/content/drive/MyDrive/Decision_Fusion_Video_Level/File_' + str(k1) + '_' + str(chunk_size)+ '_' + l_name + '.csv'
  pd.DataFrame(np.asarray(value_save)).to_csv(path)
...
x = [0.33,0.25,0.5,0.25,0,0.5,0.5]
y = [0.33,0.5,0.25,0.25,0.5,0,0.5]
z = [0.33,0.25,0.25,0.5,0.5,0.5,0]

Model_kineme, Model_AU, Model_audio, callback = model_formation(chunk_size)
k = 0
for i in range(0,1):
  print(x[i],y[i],z[i])
  model_call(file_to_work, chunk_size,l_name, Model_kineme, Model_AU, Model_audio, callback,x[i],y[i],z[i],k)
  k = k+1
